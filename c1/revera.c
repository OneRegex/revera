...
revera_locale *revera_locale_open(const char *name, size_t name_len,
    ...
    revera_eng_Tup_t4c6f63616c65x_bool result =
        revera_eng_LocaleOpen(&mem, (vg_str){locale_blob, (int64_t)sizeof(locale_blob)},
...
static bool load_base(revera_eng_Locale *base) {
    revera_eng_Tup_t4c6f63616c65x_bool result =
        revera_eng_LocaleLoad((vg_str){locale_blob, (int64_t)sizeof(locale_blob)});
    if (!result.r1) {
        return false;
    }
    *base = result.r0;
