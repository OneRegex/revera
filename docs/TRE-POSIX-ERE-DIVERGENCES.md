...
The implementation baseline is `ref/tre` commit `71bfcaf0af3994384987c6c2679ed7d078ffe189`, which identifies itself as TRE 0.9.0.
TRE's own README still names IEEE Std 1003.1-2001 / Issue 6 as its conformance target ([`README.md`, lines 101-116](../ref/tre/README.md)).
...
TRE's [`TODO`, lines 12-15](../ref/tre/TODO) acknowledges the first two gaps as missing features.
...
TRE's bracket parser unconditionally returns `REG_ECOLLATE` as soon as an item begins with `[.` ([`lib/tre-parse.c`, lines 296-301](../ref/tre/lib/tre-parse.c)).
...
- the parser takes its default `minimal` value from `REG_UNGREEDY`, and inverts it for a following `?`.
  See [`lib/tre-parse.c`, lines 623-653 and 835-863](../ref/tre/lib/tre-parse.c).
...
See [`doc/tre-syntax.html`, lines 96-156](../ref/tre/doc/tre-syntax.html) and [`doc/tre-api.html`, lines 131-134](../ref/tre/doc/tre-api.html).
...
at [`include/tre/tre.h`, lines 92-106](../ref/tre/include/tre/tre.h).
The implementation also sets `TRE_MAX_STRING` to `INT_MAX` ([`lib/tre-internal.h`, lines 28-30](../ref/tre/lib/tre-internal.h)).
...
`--enable-system-abi` is optional, and [`configure.ac`, lines 121-127](../ref/tre/configure.ac) disables it by default.
...
The first branch of `tre_fill_pmatch()` skips submatch construction, after which its final loop writes the unused entries ([`lib/regexec.c`, lines 54-114](../ref/tre/lib/regexec.c)).
`tre_match()` calls this function after every successful match without overriding `nmatch` when `REG_NOSUB` applies ([`lib/regexec.c`, lines 139-201](../ref/tre/lib/regexec.c)).
...
For ordinary members and ranges, the bracket parser adds opposite-case counterpart items to the positive list, at [`lib/tre-parse.c`, lines 380-414](../ref/tre/lib/tre-parse.c).
It then builds the negated union from that expanded list, at [`lib/tre-parse.c`, lines 441-457](../ref/tre/lib/tre-parse.c).
Negated character classes take a separate path with the same effect.
The matcher rejects a character when either of its cases belongs to the class, at [`lib/tre-match-utils.h`, lines 204-215](../ref/tre/lib/tre-match-utils.h).
...
The escape macros are enumerated in [`lib/tre-parse.c`, lines 53-63](../ref/tre/lib/tre-parse.c).
The parser implementations for quoting, assertions, numeric escapes, and ERE back-references are at [`lib/tre-parse.c`, lines 1383-1530](../ref/tre/lib/tre-parse.c).
Approximate and omitted-bound parsing is at [`lib/tre-parse.c`, lines 615-863](../ref/tre/lib/tre-parse.c).
Inline groups are handled at [`lib/tre-parse.c`, lines 1217-1376](../ref/tre/lib/tre-parse.c).
...
This behavior comes from the parser's explicit empty-node paths ([`lib/tre-parse.c`, lines 1357-1376 and 1620-1651](../ref/tre/lib/tre-parse.c)).
...
This contradicts TRE's syntax manual, which says that `{` followed by a non-digit is ordinary ([`doc/tre-syntax.html`, lines 368-371](../ref/tre/doc/tre-syntax.html)).
...
TRE forms ranges from numeric character values and rejects a descending pair ([`lib/tre-parse.c`, lines 284-301](../ref/tre/lib/tre-parse.c)).
...
The definitions are in [`include/tre/tre.h`, lines 54-90 and 109-161](../ref/tre/include/tre/tre.h).
...
[`include/tre/tre.h`, lines 178-301](../ref/tre/include/tre/tre.h) declares them.
