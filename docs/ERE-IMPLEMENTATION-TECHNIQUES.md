...
RE2 uses backtracking-style submatch priority in its NFA on purpose, at [`re2/nfa.cc`, lines 7-22](../ref/re2/re2/nfa.cc).
MinRX says that its structured-NFA algorithm is not yet proven correct, at [`ALGORITHM.txt`, lines 435-451](../ref/minrx/ALGORITHM.txt).
...
See [`lib/tre-internal.h`, lines 241-263](../ref/tre/lib/tre-internal.h) and [`lib/tre-match-parallel.c`, lines 155-205](../ref/tre/lib/tre-match-parallel.c).
MinRX usually borrows reusable scratch storage.
It falls back to private scratch when another execution already owns that storage, at [`minrx.c`, lines 2026-2087](../ref/minrx/minrx.c).
...
TRE computes nullable, first-position, and last-position sets using an explicit stack rather than recursive calls ([`lib/tre-compile.c`, lines 1305-1558](../ref/tre/lib/tre-compile.c)).
...
See [`lib/tre-compile.c`, lines 1782-1832](../ref/tre/lib/tre-compile.c).
...
RE2 simplifies counted repetitions and character classes before lowering, then optimizes and flattens its instruction graph ([`re2/compile.cc`, lines 1115-1197](../ref/re2/re2/compile.cc)).
Its flattening partitions epsilon-connected instructions into lists so repeated traversals do not expand the program quadratically ([`re2/prog.cc`, lines 528-658](../ref/re2/re2/prog.cc)).
...
TRE allocates small objects from blocks and frees all blocks together ([`lib/tre-mem.c`, lines 9-13 and 90-152](../ref/tre/lib/tre-mem.c)).
It also uses a bounded growable explicit stack ([`lib/tre-stack.c`, lines 66-102](../ref/tre/lib/tre-stack.c)).
...
MinRX sorts and coalesces ranges at finalization, then uses binary search for membership ([`charset.c`, lines 8905-8926 and 8943-9014](../ref/minrx/charset.c)).
RE2 goes further for a byte program.
It maps bytes that every instruction treats alike to one equivalence class, at [`re2/prog.cc`, lines 452-518](../ref/re2/re2/prog.cc).
...
See [`charset.c`, lines 28-32 and 9237-9249](../ref/minrx/charset.c).
...
TRE states this directly and gives linear input-length worst-case behavior for a fixed TNFA ([`lib/tre-match-parallel.c`, lines 9-22](../ref/tre/lib/tre-match-parallel.c)).
...
See [`lib/tre-match-parallel.c`, lines 280-334 and 362-496](../ref/tre/lib/tre-match-parallel.c).
...
See [`lib/tre-compile.c`, lines 140-166 and 222-304](../ref/tre/lib/tre-compile.c) and [`lib/tre-internal.h`, lines 201-205 and 227-263](../ref/tre/lib/tre-internal.h).
At a state merge, its matcher compares the ordered tag vectors.
It does not simply keep the first path it met, at [`lib/tre-match-parallel.c`, lines 433-491](../ref/tre/lib/tre-match-parallel.c).
...
See [`ALGORITHM.txt`, lines 80-159](../ref/minrx/ALGORITHM.txt).
...
See [`ALGORITHM.txt`, lines 107-131](../ref/minrx/ALGORITHM.txt).
...
It compares those counters before the ordinary POSIX priority stack, at [`ALGORITHM.txt`, lines 320-420](../ref/minrx/ALGORITHM.txt).
After it finds an accepting path, it keeps looking for a longer compatible whole match.
It rejects any path that increases an already fixed minimized count, at [`ALGORITHM.txt`, lines 424-433](../ref/minrx/ALGORITHM.txt).
...
Insertion order can also act as a work queue, at [`re2/sparse_set.h`, lines 8-30](../ref/re2/re2/sparse_set.h).
...
See [`minrx.c`, lines 373-481](../ref/minrx/minrx.c).
...
RE2 identifies capture copying as its NFA bottleneck ([`re2/onepass.cc`, lines 103-132](../ref/re2/re2/onepass.cc)).
...
See [`minrx.c`, lines 158-285](../ref/minrx/minrx.c).
...
MinRX has a separate no-sub executor whose states are only set membership ([`minrx.c`, lines 2569-2748](../ref/minrx/minrx.c)).
TRE similarly sets its runtime tag count to zero when its caller passes no tag buffer ([`lib/tre-match-parallel.c`, lines 155-174](../ref/tre/lib/tre-match-parallel.c)).
...
TRE derives a unique possible first byte and uses `memchr()` before starting the TNFA ([`lib/tre-match-parallel.c`, lines 211-236](../ref/tre/lib/tre-match-parallel.c)).
...
See [`minrx.c`, lines 1858-1953 and 2439-2498](../ref/minrx/minrx.c).
RE2 extracts a required literal prefix and chooses a specialized prefix search.
See [`re2/regexp.cc`, lines 734-759](../ref/re2/re2/regexp.cc) and [`re2/prog.cc`, lines 1020-1037](../ref/re2/re2/prog.cc).
...
It precomputes the next state, empty-width conditions, and capture actions in a compact table ([`re2/onepass.cc`, lines 7-51 and 124-168](../ref/re2/re2/onepass.cc)).
...
See [`re2/dfa.cc`, lines 1280-1313](../ref/re2/re2/dfa.cc).
Its cache has a hard memory budget and returns failure rather than allocating beyond it ([`re2/dfa.cc`, lines 743-799](../ref/re2/re2/dfa.cc)).
The caller can then fall back to another executor.
RE2 routes among its DFA, one-pass, bit-state and NFA engines this way, at [`re2/re2.cc`, lines 730-904](../ref/re2/re2/re2.cc).
...
See [`re2/bitstate.cc`, lines 7-18 and 88-102](../ref/re2/re2/bitstate.cc).
...
See [`lib/regexec.c`, lines 138-199](../ref/tre/lib/regexec.c).
