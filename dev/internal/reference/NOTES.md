...
- [x] Module `revera` created, now the package `dev/internal/reference`.
- [x] Locale data generator (`dev/internal/genlocale`) and `locale` package.
...
- [x] The libc differential harness is a cgo test now (`libc_differential_test.go`, macOS only, with the C wrapper in `libcre`).
...
- The tables come from the same CLDR 48.2 data as the C implementation.
  A generator parses `locale/rv_locale_data.inc` at the repository root and writes `locale/data.bin`.
