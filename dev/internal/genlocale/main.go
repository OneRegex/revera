// Command genlocale converts the generated C locale tables into the compact binary blob that every engine embeds.
//
// The input is locale/rv_locale_data.inc at the repository root.
// Without arguments, the command finds the repository from the working directory and writes every checked-in copy of data.bin.
// With arguments, the first names the input and the rest name the output files.
...
func main() {
	...
	if len(os.Args) > 1 {
		...
		repo, err := conformance.FindRepositoryRoot(cwd)
		if err != nil {
			fmt.Fprintln(os.Stderr, "genlocale:", err)
			os.Exit(1)
		}
	...
	if len(outputs) == 0 {
		fmt.Fprintln(os.Stderr, "usage: genlocale [input.inc output.bin...]")
		os.Exit(2)
	}
