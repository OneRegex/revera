...
// DiscoverBackends loads every backend.json one level below the repository root or below native/, sorted by name.
func DiscoverBackends(repo string) ([]*Backend, error) {
	var matches []string
	for _, pattern := range []string{filepath.Join(repo, "*", "backend.json"), filepath.Join(repo, "native", "*", "backend.json")} {
