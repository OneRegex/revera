...
// The crosscheck and probecheck programs and the conform command are thin layers over it.
package conformance
...
func enc(s string) string {
	return protocol.DriverEncode(s)
}
...
func (b *builder) fixed() {
	for _, cflags := range protocol.FixedFlagSets {
		for _, pattern := range protocol.FixedPatterns {
			b.compile(cflags, pattern)
			if !b.light || pattern != protocol.HeavyPattern {
...
func (b *builder) replace() {
	...
	// The reference engine has no positioned errors to compare there.
	cases := append([]protocol.ReplaceCase{}, protocol.ReplaceCases...)
	cases = append(cases,
		protocol.ReplaceCase{Pattern: "(a+)(b+)", Subject: "aabb xab", Replacement: `\9`, Limit: -1},
		protocol.ReplaceCase{Pattern: "a", Subject: "aaaa", Replacement: `x\`, Limit: -1})
...
func (b *builder) locales() {
	...
	for _, n := range names {
		...
		b.add("O 0 2048")
		b.add("O 7680 8192")
		for _, p := range protocol.LocalePatterns {
			b.compile(revera.FlagICase, p)
			for _, s := range protocol.LocaleSubjects {
				b.exec(0, s)
			}
		}
		if n == "nosuchlocale" {
...
func Answer(commands []string) Corpus {
	session := protocol.NewDriverSession()
