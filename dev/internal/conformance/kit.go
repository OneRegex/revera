...
func (r *run) checkGenerated() Result {
	start := time.Now()
	out, err := command(filepath.Join(r.opts.Repo, "dev"), "go", "run", "./cmd/generate", "-check")
	if err != nil {
...
func LocaleDataProblems(repo string) []string {
	...
	for _, rel := range localeDataCopies {
		...
		if err != nil {
			...
			problems = append(problems, rel+" differs from go/data.bin")
		}
	}
...
func (r *run) prepare(backends []*Backend) Result {
	...
	if !r.skip("fuzz") {
		...
		if err := protocol.WriteFuzzPack(&buf, r.seeds); err != nil {
			return r.record(repoScope, "corpus", Failed, start, err.Error())
		}
