...
Every theorem here is about the exact JSON artifacts that the Go, Rust, Zig and C++ printers consume.
Those artifacts are embedded byte for byte.
Every theorem is also about the formal Vego semantics of Interp.lean.
They are proved by native evaluation (`native_decide`), so the trusted base is the Lean kernel plus the Lean compiler.
...
3. `revera_corpus_agrees_within_contract`.
   ...
   The theorem leaves their 1056 executions out of the 86691 commands.
