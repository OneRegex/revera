...
// This package is the Vego rewrite of the go0 revera engine: a
// cleanroom implementation of the POSIX.1-2024 Issue 8 Extended
// Regular Expression language, as specified by
// docs/POSIX-1-2024-ERE-SPECIFICATION.md in this repository.
//
// The API mirrors the regcomp() family. Patterns and subjects are
// UTF-8, length-delimited strings. NUL is an ordinary character;
// this input lies outside the C-representable domain, so it is a
// permitted extension. Dot never matches NUL. Bytes that are not
// valid UTF-8 match nothing.
//
// The subset has no methods, so every operation is a function that
// takes the Regexp as its first argument. Compile returns the
// Regexp by value; treat it as immutable afterwards. Exec never
// writes to it, so concurrent calls on one compiled value are safe
// in the Go instantiation.

// Regexp is a compiled regular expression.
type Regexp struct {
	// nodes is the AST arena; root indexes it.
	nodes []node
	// brackets is the bracket arena; opBracket nodes index it.
	brackets []bracketSet
	root     int32
	nsub     int
	flags    uint32
	loc      Locale
	// minSlots counts shortest-preferring repetitions. Each holds
	// one counter slot, assigned in pattern pre-order.
	minSlots int
	// nested lists, for every group, the groups strictly inside it.
	nested [][]int32
	// multi is true when some bracket can consume several
	// characters.
	multi bool
	// progOK is false when interval expansion passed the program
	// size cap. minLen then holds the smallest possible match
	// length in characters, for the fallback in Exec.
	progOK bool
	prog   program
	minLen int
...
// compileScan walks the pattern in pre-order and fills the
// tree-derived Regexp fields: the minimal counter slots, the
// multi-character flag, and the anchor flag.
...
// groupStack tracks the open groups while collectNested walks the
// pattern.
type groupStack struct {
	g []int32
}

// collectNested records, for each group, the group numbers inside
// it.
...
// Compile parses and compiles pattern in the given locale. The
// POSIX locale is LocalePOSIX(). A nonzero code in the returned
// Error means failure; the Regexp is then meaningless.
...
// NumSub returns the number of parenthesized subexpressions, like
// re_nsub.
func NumSub(re *Regexp) int {
	return re.nsub
}

// trivialNullMatch reports whether existence alone answers an Exec
// call: a nullable, anchor-free pattern matches the null string at
// the start of any subject, and the caller asked for no offsets.
...
// element 0 is the whole match, element i is capturing subexpression
// i, and every remaining element is set to -1, -1. When the
// expression was compiled with FlagNoSub, or pmatch is empty, pmatch
// stays untouched. It returns false when no match exists; the Error
// code is then ErrNone for a plain mismatch.
func Exec(re *Regexp, subject string, pmatch []Match, eflags uint32) (bool, Error) {
	...
	if !re.progOK {
		...
		// longer one would really need the huge program. The
		// matcherContract fallback branch mirrors this path's cost.
		if len(subject) < re.minLen {
			return false, noError()
		}
		if runeCount(subject) < re.minLen {
			// The byte count passed, but lengths compare in
			// characters.
			return false, noError()
		}
	...
	if re.prog.failMin != failMinNone &&
		...
		// enough to reach it, so the program may miss matches or
		// pick the wrong spans. Pruning removes possibilities and
		// adds none, so a match the program still finds proves
		// existence; a miss proves nothing, and offsets would need
		// the full program.
		if trivialNullMatch(re, pmatch) {
			return true, noError()
		}
