...
// Hand-written UTF-8 handling. The subset imports nothing, so the
// few operations the engine needs are spelled out here. The decoder
// matches the Go standard decoder bit for bit: strict validation,
// no overlong forms, no surrogates, and every invalid byte reports
// the invalid sentinel with size one.

// invalidRune marks a byte that is not valid UTF-8. It matches
// nothing. An encoded U+FFFD decodes with size 3, so the two stay
// distinguishable.
...
// It returns the character and its byte size, or invalidRune and
// size one when the bytes are not one valid UTF-8 sequence.
...
// runeCount counts the characters of s. Each invalid byte counts as
// one character, like the standard rune counter.
