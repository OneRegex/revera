...
func BenchCases() []BenchCase {
	...
	cases := []BenchCase{
		...
		{Name: "icase-utf8", Group: "compile", Kind: BenchCompile, Flags: FlagICase, Pattern: "é(è|e)*", Iters: 10000},
		...
		{Name: "nosub-long", Group: "match", Kind: BenchMatch, Flags: FlagNoSub, Pattern: "([a-z]+)-([a-z]+)-([0-9]+)", Subject: text + " alpha-beta-42", Iters: 500},
		...
		{Name: "icase", Group: "match", Kind: BenchMatch, Flags: FlagICase, Pattern: "NEEDLE", Subject: hit, Iters: 1000},
		{Name: "newline", Group: "match", Kind: BenchMatch, Flags: FlagNewline, Pattern: "^[a-z]+$", Subject: strings.ReplaceAll(text, ". ", "\n") + "\nneedle", Iters: 1000},
		{Name: "minimal", Group: "match", Kind: BenchMatch, Flags: FlagMinimal, Pattern: "(a*?)c", Subject: aaa(100) + "c", Iters: 5000},
		...
		{Name: "tr-icase", Group: "match", Kind: BenchMatch, Locale: "tr", Flags: FlagICase, Pattern: "[[:alpha:]]+", Subject: "IİıiIi kelime", Iters: 5000},
...
func MustEmbeddedLocale() Locale {
...
func LocaleByName(base *Locale, name string) (Locale, bool) {
	if name == "" {
		return LocalePOSIX(), true
	}
	return LocaleSelect(base, name, "")
}
...
func NewBenchSession() *BenchSession {
	return &BenchSession{base: MustEmbeddedLocale(), cur: LocalePOSIX()}
}
...
func (s *BenchSession) Eval(line string) string {
	...
	case "B":
		...
		re, err := Compile(req.Pattern, s.cur, req.Flags)
		if err.Code != ErrNone {
		...
		case BenchCompile:
			loc := s.cur
			op = func() { _, _ = Compile(req.Pattern, loc, req.Flags) }
		case BenchMatch:
			groups := NumSub(&re) + 1
			op = func() {
				pmatch := make([]Match, groups)
				_, _ = Exec(&re, req.Subject, pmatch, 0)
			}
		case BenchReplace:
			op = func() { _, _ = ReplaceAll(&re, req.Subject, req.Replacement, -1, 0) }
		default:
