...
// The Go fuzz test adds a differential check against go0 on top of it.
//
// Layout of an input:
//
...
func FuzzRun(base *Locale, data []byte) {
	...
	re, err := Compile(in.Pattern, loc, in.Flags)
	if err.Code != ErrNone {
		return
	}
	pmatch := make([]Match, NumSub(&re)+1)
	_, _ = Exec(&re, in.Subject, pmatch, in.EFlags)
	_, _ = ReplaceAll(&re, in.Subject, in.Replacement, -1, in.EFlags)
	it, ierr := MatchIterInit(&re, 3)
	if ierr.Code == ErrNone {
		for {
			got, nerr := MatchIterNext(&re, &it, in.Subject, in.EFlags, pmatch)
			if nerr.Code != ErrNone || !got {
				break
			}
		}
	}
	c := ContractFor(&re, len(in.Subject))
...
func FuzzSeeds() [][]byte {
	...
	for _, flags := range []uint32{0, FlagICase, FlagMinimal} {
	...
	for _, pattern := range LocalePatterns {
		for _, subject := range LocaleSubjects {
			add(FuzzInput{Flags: FlagICase, Locale: "tr", Pattern: pattern, Subject: subject, Replacement: `\0`})
		}
	}
