...
// Usage:
//
//	bench [-repo path] [-backend dir]... [-reps n] [-scale f] [-go0] [-build] [-only prefix] [-tsv file]
...
func run(args []string, stdout, stderr io.Writer) int {
	...
	withGo0 := flags.Bool("go0", false, "also time the go0 reference engine")
	...
	columns := []column{{name: "go1", by: evalInProcess(revera.NewBenchSession(), commands, cases, *reps)}}
	if *withGo0 {
		columns = append(columns, column{name: "go0", by: evalInProcess(newGo0Session(), commands, cases, *reps)})
	}
	for _, b := range backends {
...
func selectCases(only string, scale float64) []revera.BenchCase {
	var out []revera.BenchCase
	for _, c := range revera.BenchCases() {
...
func benchCommands(cases []revera.BenchCase, reps int) []string {
	var lines []string
	for _, c := range cases {
		if c.Locale == "" {
			lines = append(lines, "P")
		} else {
			lines = append(lines, "L "+revera.DriverEncode(c.Locale)+" -")
		}
		lines = append(lines, revera.BenchCommand(c, reps))
	}
	return lines
}
...
func evalInProcess(s session, commands []string, cases []revera.BenchCase, reps int) map[string]measurement {
...
func evalBinary(path string, commands []string, cases []revera.BenchCase, reps int) (map[string]measurement, error) {
...
func collect(commands, answers []string, cases []revera.BenchCase, reps int) (map[string]measurement, error) {
	...
	for i, line := range commands {
		...
		case 'B':
			r, err := revera.ParseBenchResult(answers[i])
			if err != nil {
				return nil, err
			}
...
func measure(r revera.BenchResult, iters int) measurement {
...
func contractsFor(cases []revera.BenchCase) map[string]int64 {
	base := revera.MustEmbeddedLocale()
	out := map[string]int64{}
	for _, c := range cases {
		if c.Kind != revera.BenchMatch {
			continue
		}
		loc, ok := revera.LocaleByName(&base, c.Locale)
		if !ok {
			continue
		}
...
func printHeader(w io.Writer, repo string, backends []*conformance.Backend, cases []revera.BenchCase, reps int, scale float64) {
...
func printTables(w io.Writer, cases []revera.BenchCase, columns []column, contracts map[string]int64) {
	for _, g := range revera.BenchGroups {
		header := []string{"case"}
...
func writeTSV(path string, cases []revera.BenchCase, columns []column, contracts map[string]int64, reps int) error {
