...
func TestReservedPackageNameProducesValidNamespace(t *testing.T) {
	...
	for namespace, want := range cases {
		g := &gen{p: &vegoc.Program{}, hppName: "engine.hpp", ns: namespace}
...
func TestRuntimePackageNamespaceCompiles(t *testing.T) {
	...
	arena := &vegoc.StructDecl{Name: "Arena", Fields: []vegoc.Param{{Name: "Value", Type: vegoc.TInt}}}
	p := &vegoc.Program{Package: "vg", Types: []*vegoc.StructDecl{arena},
		StructMap: map[string]*vegoc.StructDecl{"Arena": arena}}
