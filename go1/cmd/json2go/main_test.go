...
func TestRenderValidatesDocumentHeader(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := render([]byte(tc.src)); err == nil || !strings.Contains(err.Error(), tc.want) {
...
func TestRenderMinimalDocument(t *testing.T) {
	src, err := render([]byte(`{"vego": 1, "package": "p", "consts": [], "vars": [], "types": [], "funcs": []}`))
	if err != nil {
		t.Fatal(err)
	}
