...
func TestArtifactsAndPlanAreDeterministic(t *testing.T) {
	...
	wantArtifacts := []string{
		"go1/revera.vego.json", "go1/probe.vego.json",
		"rust1/src/engine.rs", "rust1/src/probe_engine.rs",
	...
	plan := generationPlan(stage, targets)
	wantSteps := []string{
		"export revera JSON", "export probe JSON",
...
func TestFindRepositoryRoot(t *testing.T) {
	...
	deep := filepath.Join(repo, "go1", "cmd", "nested")
	if err := os.MkdirAll(deep, 0o755); err != nil {
		t.Fatal(err)
	}
	decoy := filepath.Join(repo, "go1", "cmd")
...
func TestGenerationFailureIsTransactional(t *testing.T) {
	...
	steps := generationPlan("stage", targets)
...
func TestMissingGeneratorOutputIsTransactional(t *testing.T) {
	...
	original := seedArtifacts(t, repo, artifacts, false)
	runner := successfulFakeRunner()
	runner.omit["cpp1/engine.cpp"] = true
	_, err := executeWorkflow(repo, targets, false, runner, io.Discard)
	if err == nil || !strings.Contains(err.Error(), "did not create cpp1/engine.cpp") {
...
func TestGenerationReplacesDestinationSymlink(t *testing.T) {
	...
	destination := filepath.Join(repo, "rust1", "src", "engine.rs")
	...
	for _, level := range []string{"top-level", "nested"} {
		for _, check := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				link := filepath.Join(repo, "rust1", "src")
				if level == "top-level" {
					outsideParent = outsideRoot
					link = filepath.Join(repo, "rust1")
				}
...
func TestCheckGeneratedDetectsContentProblems(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(repo, "cpp1", "engine.cpp"), []byte("stale\r\n"))
	if err := os.Remove(filepath.Join(repo, "rust1", "src", "engine.rs")); err != nil {
		t.Fatal(err)
	}
	writeTestFile(t, filepath.Join(repo, "zig1", "src", "engine.zig"), nil)
	result, err = executeWorkflow(repo, targets, true, successfulFakeRunner(), io.Discard)
	if err != nil {
		t.Fatal(err)
	}
	want := []artifactProblem{
		{kind: "stale", rel: "cpp1/engine.cpp"},
		{kind: "missing", rel: "rust1/src/engine.rs"},
		{kind: "stale", rel: "zig1/src/engine.zig"},
	}
	if !reflect.DeepEqual(result.problems, want) {
		t.Fatalf("problems = %#v, want %#v", result.problems, want)
	}
	if _, err := os.Stat(filepath.Join(repo, "rust1", "src", "engine.rs")); !errors.Is(err, os.ErrNotExist) {
...
func TestCheckGeneratedHonorsTargetSelection(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(repo, "zig1", "src", "engine.zig"), []byte("stale\n"))
...
func TestRunExitCodesAndDiagnostics(t *testing.T) {
	...
	runner := successfulFakeRunner()
	if code := run(nil, ".", &stdout, &stderr, runner); code != 2 {
		t.Fatalf("empty invocation exit = %d, want 2", code)
	}
	stdout.Reset()
	stderr.Reset()
	...
	stdout.Reset()
	stderr.Reset()
	if code := run([]string{"generate", "-target", "Rust"}, ".", &stdout, &stderr, runner); code != 2 {
		t.Fatalf("bad target exit = %d, want 2", code)
	}
	stdout.Reset()
	stderr.Reset()
	if code := run([]string{"generate", "operand"}, ".", &stdout, &stderr, runner); code != 2 {
	...
	if code := run([]string{"generate"}, filepath.Dir(actualRepo), &stdout, &stderr, runner); code != 1 {
	...
	stdout.Reset()
	stderr.Reset()
	if code := run([]string{"check-generated", "-repo", repo, "-target", "rust"}, repo,
		&stdout, &stderr, successfulFakeRunner()); code != 1 {
		t.Fatalf("stale check exit = %d, want 1", code)
	}
	if !strings.Contains(stderr.String(), "stale: go1/revera.vego.json") ||
		!strings.Contains(stderr.String(), "revera generate -target rust") {
		t.Fatalf("stale diagnostics are not actionable: %q", stderr.String())
	}
	stdout.Reset()
	stderr.Reset()
	if code := run([]string{"generate", "-repo", repo, "-target", "rust"}, repo,
