...
Goal: from the work in go1/, write printers and implementations in Zig, C++ and Rust, and verify they are correct.
...
- Wrote go1/vegoc (loader, checker, analyses).
...
- Zig target done and verified: json2zig printer, zig1/ runtime (vg.zig), driver, build.zig.
...
- C++ target done and verified (full corpus 86059/86059): json2cpp printer, cpp1/ runtime, driver, Makefile.
...
- Rust target done and verified (full corpus 86059/86059): json2rust printer, rust1/ crate.
...
- go1/probe is a second Vego package covering exactly those engine-unused constructs.
  cmd/proberef prints the Go results.
  The target probe binaries print the same lines, and cmd/probecheck diffs them.
...
- A second swival round returned four findings, and probe coverage now backs every fix.
  A package variable may not contain a slice at any depth, so vego2json and vegoc reject one, because a global is static constant data.
...
- Only C++ needed a printer change.
  ...
  `json2cpp` took a `-ns` flag, and the engine moved to `namespace revera::engine`.
