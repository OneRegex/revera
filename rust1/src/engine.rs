// Code generated by json2rust from the Vego JSON form. DO NOT EDIT.

#![allow(non_snake_case, non_camel_case_types, non_upper_case_globals)]
#![allow(unused_parens, unused_mut, unused_variables, dead_code)]
#![allow(unreachable_code, unused_assignments, unused_unsafe)]
    ...
    loop {
        ...
        if ((((item.kind == itemChar) && (peekByte(p) == 45u8)) && ({ let _t6 = 1i64; peekByteAt(p, _t6) } != 93u8)) && ((p.pos + 1i64) < p.src.len)) {
            ...
            if (end.kind != itemChar) {
                return { let _t9 = ErrERange; let _t10 = rangeStart; fail(p, _t9, _t10) };
            }
        ...
        if ((((item.kind != itemChar) && (peekByte(p) == 45u8)) && ({ let _t18 = 1i64; peekByteAt(p, _t18) } != 93u8)) && ((p.pos + 1i64) < p.src.len)) {
            return { let _t19 = ErrERange; let _t20 = p.pos; fail(p, _t19, _t20) };
        }
        {
            let _t21 = item.kind;
            if _t21 == itemChar {
                    ...
                    '_b2: while (lo < n) {
                        '_c2: {
                            ...
                            while ((i < mid) && (j < hi)) {
                                ...
                                if less {
                                    unsafe { (*tmp.ptr(w)) = rr.get(i); }
                                    i += 1i64;
                                } else {
                                    unsafe { (*tmp.ptr(w)) = rr.get(j); }
                                    j += 1i64;
                                }
                                w += 1i64;
                            }
            ...
            '_c3: {
                if (b.ranges.get(i_2).lo <= (b.ranges.get(w).hi + 1i32)) {
                    if (b.ranges.get(i_2).hi > b.ranges.get(w).hi) {
                        unsafe { (*b.ranges.ptr(w)).hi = b.ranges.get(i_2).hi; }
                    }
                } else {
                    w += 1i64;
                    unsafe { (*b.ranges.ptr(w)) = b.ranges.get(i_2); }
                }
            }
            i_2 += 1i64;
        }
...
pub fn setMatch(caps: vg::Slice<Match>, idx: i64, so: i64, eo: i64) {
    unsafe { (*caps.ptr(idx)).So = so; }
    unsafe { (*caps.ptr(idx)).Eo = eo; }
}
...
pub fn addCounters(s: &mut capSolver, re: &mut Regexp, t: i32, out: vg::Slice<i64>) {
    ...
    if ((re.nodes.get(((ni) as i64)).op == opRepeat) && re.nodes.get(((ni) as i64)).minimal) {
        unsafe { (*out.ptr(re.nodes.get(((ni) as i64)).index)) += (((s.trees.get(((t) as i64)).j - s.trees.get(((t) as i64)).i)) as i64); }
    }
    ...
    let count: i64 = ((s.trees.get(((t) as i64)).kidsLen) as i64);
    {
        ...
        '_b1: while (k < count) {
            '_c1: {
                { let _t2 = s.kidStore.get((off + k)); let _t3 = out; addCounters(s, re, _t2, _t3) };
            }
            k += 1i64;
        }
    }
    ...
    if (re.minSlots > 0i64) {
        {
            ...
            '_b1: while (idx < re.minSlots) {
                '_c1: {
                    unsafe { (*s.ctrA.ptr(idx)) = 0i64; }
                    unsafe { (*s.ctrB.ptr(idx)) = 0i64; }
                }
                idx += 1i64;
            }
        }
...
pub fn compactQueue(ws: &mut engineWS) {
        ...
        '_b1: while (r < ws.queue.len) {
            '_c1: {
                ...
                if (ws.onq.get(((pc) as i64)) == 0u8) {
                    unsafe { (*ws.onq.ptr(((pc) as i64))) = 1u8; }
                    unsafe { (*ws.queue.ptr(w)) = pc; }
                    w += 1i64;
                }
            }
            r += 1i64;
    ...
    ws.queue = ws.queue.head(w);
    {
        ...
        '_b2: while (i < ws.queue.len) {
            '_c2: {
                unsafe { (*ws.onq.ptr(((ws.queue.get(i)) as i64))) = 0u8; }
            }
            i += 1i64;
        }
    }
...
pub fn paArrive(mem: &vg::Arena, e: &mut phaseAState, ws: &mut engineWS, re: &mut Regexp, pc: u32, delta: i64, start: i32, ctr: vg::Slice<u32>) {
    ...
    if (ws.slots.get(fi).r#gen != g) {
        unsafe { (*ws.slots.ptr(fi)).r#gen = g; }
        unsafe { (*ws.slots.ptr(fi)).active = ws.slots.get(fi).active.head(0i64); }
    }
...
pub fn paRun(mem: &vg::Arena, e: &mut phaseAState, ws: &mut engineWS, re: &mut Regexp) {
    ...
    loop {
        ...
        unsafe { (*ws.slots.ptr(si)).r#gen = g; }
        let mut w: i64 = 0i64;
        {
            let mut r: i64 = 0i64;
            '_b1: while (r < ws.slots.get(si).active.len) {
                '_c1: {
                    let pc: u32 = ws.slots.get(si).active.get(r);
                    if (ws.slots.get(si).stamp.get(((pc) as i64)) == g) {
                        unsafe { (*ws.slots.get(si).active.ptr(w)) = pc; }
                        w += 1i64;
                    }
                }
                r += 1i64;
        ...
        unsafe { (*ws.slots.ptr(si)).active = ws.slots.get(si).active.head(w); }
        if ((((w == 0i64) && (!e.matched)) && re.prog.scan.enabled) && (e.pos < e.subject.len)) {
            if (!{ let _t2 = prev; bolAt(e, _t2) }) {
        ...
        if (!atEnd) {
            {
                let _t3 = decodeRuneAt(e.subject, e.pos);
                e.cur = _t3.0;
                e.size = _t3.1;
            }
        }
...
pub fn memoInsert(t: &mut memoTab, k: memoKey, v: memoVal) {
    ...
    unsafe { (*t.used.ptr(((at) as i64))) = 1u8; }
    unsafe { (*t.keys.ptr(((at) as i64))) = k; }
    unsafe { (*t.vals.ptr(((at) as i64))) = v; }
    t.count += 1i64;
}
        ...
        '_b1: while (idx < pmatch.len) {
            '_c1: {
                if ((idx < caps.len) && (caps.get(idx).So >= 0i64)) {
                    unsafe { (*pmatch.ptr(idx)).So = d.byteAt.get(caps.get(idx).So); }
                    unsafe { (*pmatch.ptr(idx)).Eo = d.byteAt.get(caps.get(idx).Eo); }
                } else {
                    unsafe { (*pmatch.ptr(idx)).So = (1i64).wrapping_neg(); }
                    unsafe { (*pmatch.ptr(idx)).Eo = (1i64).wrapping_neg(); }
                }
            }
            idx += 1i64;
        }
...
pub fn disjointFirsts(mem: &vg::Arena, nodes: vg::Slice<node>, ni: i32) -> bool {
    ...
    unsafe { (*nodes.ptr(((ni) as i64))).firsts = firsts; }
    return true;
}
...
pub fn buildScanFilter(mem: &vg::Arena, pr: &mut program, newlineMode: bool) {
    ...
    while (stack.len > 0i64) {
        ...
        unsafe { (*seen.ptr(((pc) as i64))) = true; }
        {
...
pub fn patch(b: &mut progBuilder, slots: vg::Slice<patchSlot>, target: u32) {
    ...
    }
    {
        ...
        '_b1: while (i < slots.len) {
            '_c1: {
                if slots.get(i).alt {
                    unsafe { (*b.prog.ins.ptr(((slots.get(i).idx) as i64))).alt = target; }
                } else {
                    unsafe { (*b.prog.ins.ptr(((slots.get(i).idx) as i64))).next = target; }
                }
            }
            i += 1i64;
        }
    ...
    let count: i64 = nodes.get(((ni) as i64)).ch.len;
    {
        ...
        '_b1: while (i < count) {
            '_c1: {
                ...
                if (i < (count - 1i64)) {
                    unsafe { (*b.prog.ins.ptr(((splits.get(i)) as i64))).next = sub.start; }
                } else {
                    unsafe { (*b.prog.ins.ptr(((splits.get((i - 1i64))) as i64))).alt = sub.start; }
                }
    ...
    let mut skips: vg::Slice<patchSlot> = vg::make_cap::<patchSlot>(mem, 0i64, std::cmp::max((hi - lo), 1i64));
    {
        ...
        '_b21: while (i_2 < hi) {
            '_c21: {
                ...
                unsafe { (*b.prog.ins.ptr(((split) as i64))).next = sub.start; }
                ...
                piece.start = split;
                piece.out = sub.out;
                haveResult = { let _t27 = haveResult; let _t28 = piece; fragAppend(b, &mut result, _t27, _t28) };
            }
            i_2 += 1i64;
        }
    }
...
pub fn compileScan(re: &mut Regexp, ni: i32) {
    {
        ...
        if _t1 == opRepeat {
            if re.nodes.get(((ni) as i64)).minimal {
                unsafe { (*re.nodes.ptr(((ni) as i64))).index = re.minSlots; }
                re.minSlots += 1i64;
            }
        } else if _t1 == opBracket {
    ...
    }
    {
        ...
        '_b2: while (i < re.nodes.get(((ni) as i64)).ch.len) {
            '_c2: {
                { let _t3 = re.nodes.get(((ni) as i64)).ch.get(i); compileScan(re, _t3) };
            }
            i += 1i64;
        }
    }
    ...
    while (it.pos <= subject.len) {
            ...
            '_b8: while (idx < pmatch.len) {
                '_c8: {
                    if (pmatch.get(idx).So >= 0i64) {
                        unsafe { (*pmatch.ptr(idx)).So += it.pos; }
                        unsafe { (*pmatch.ptr(idx)).Eo += it.pos; }
                    }
                }
                idx += 1i64;
            }
        ...
        if (eo > so) {
            ...
            if (so >= subject.len) {
                ...
                let _t9 = decodeRuneAt(subject, so);
                let size: i64 = _t9.1;
                it.pos = (so + size);
            }
        }
...
pub fn parseExpr(mem: &vg::Arena, p: &mut parser, loc: &mut Locale) -> i32 {
    ...
    let mut primary: i32 = vg::zero();
    {
        ...
        if _t7 == 40u8 {
            ...
            p.pos += 1i64;
            if eof(p) {
                return { let _t12 = ErrEEscape; let _t13 = start; fail(p, _t12, _t13) };
            }
            let r: i32 = nextRune(p);
            if (r < 0i32) {
                return (1i32).wrapping_neg();
...
pub fn computeLengths(mem: &vg::Arena, nodes: vg::Slice<node>, loc: &mut Locale, brackets: vg::Slice<bracketSet>, ni: i32) {
    ...
    }
    {
        ...
        if _t5 == opChar || _t5 == opAny {
            unsafe { (*nodes.ptr(((ni) as i64))).minL = 1i64; }
            unsafe { (*nodes.ptr(((ni) as i64))).maxL = 1i64; }
        } else if _t5 == opBracket {
            unsafe { (*nodes.ptr(((ni) as i64))).minL = (({ let _t6 = brackets; let _t7 = nodes.get(((ni) as i64)).br; bracketMinChars(_t6, _t7, loc) }) as i64); }
            unsafe { (*nodes.ptr(((ni) as i64))).maxL = 1i64; }
            if bracketHasMultiMembers(brackets, nodes.get(((ni) as i64)).br) {
                unsafe { (*nodes.ptr(((ni) as i64))).maxL = localeMaxElementLength(loc); }
            }
        } else if _t5 == opBOL || _t5 == opEOL {
            unsafe { (*nodes.ptr(((ni) as i64))).minL = 0i64; }
            unsafe { (*nodes.ptr(((ni) as i64))).maxL = 0i64; }
        } else if _t5 == opGroup {
            let child: i32 = nodes.get(((ni) as i64)).ch.get(0i64);
            unsafe { (*nodes.ptr(((ni) as i64))).minL = nodes.get(((child) as i64)).minL; }
            ...
            unsafe { (*nodes.ptr(((ni) as i64))).sufMax = vg::make::<i64>(mem, (count + 1i64)); }
            {
                ...
                '_b8: while (k >= 0i64) {
                    '_c8: {
                        ...
                        unsafe { (*nodes.get(((ni) as i64)).sufMin.ptr(k)) = satAdd(nodes.get(((ni) as i64)).sufMin.get((k + 1i64)), nodes.get(((child_2) as i64)).minL); }
                        unsafe { (*nodes.get(((ni) as i64)).sufMax.ptr(k)) = satAdd(nodes.get(((ni) as i64)).sufMax.get((k + 1i64)), nodes.get(((child_2) as i64)).maxL); }
                    }
                    k -= 1i64;
                }
            }
            unsafe { (*nodes.ptr(((ni) as i64))).minL = nodes.get(((ni) as i64)).sufMin.get(0i64); }
            unsafe { (*nodes.ptr(((ni) as i64))).maxL = nodes.get(((ni) as i64)).sufMax.get(0i64); }
        } else if _t5 == opAlt {
            unsafe { (*nodes.ptr(((ni) as i64))).minL = lenInf; }
            unsafe { (*nodes.ptr(((ni) as i64))).maxL = 0i64; }
            {
                let mut i_2: i64 = 0i64;
                '_b9: while (i_2 < nodes.get(((ni) as i64)).ch.len) {
                    '_c9: {
                        let child_3: i32 = nodes.get(((ni) as i64)).ch.get(i_2);
                        unsafe { (*nodes.ptr(((ni) as i64))).minL = std::cmp::min(nodes.get(((ni) as i64)).minL, nodes.get(((child_3) as i64)).minL); }
                        unsafe { (*nodes.ptr(((ni) as i64))).maxL = std::cmp::max(nodes.get(((ni) as i64)).maxL, nodes.get(((child_3) as i64)).maxL); }
                    }
                    i_2 += 1i64;
                }
            }
            ...
            unsafe { (*nodes.ptr(((ni) as i64))).minL = satMul(nodes.get(((ni) as i64)).min, nodes.get(((child_4) as i64)).minL); }
            if ((nodes.get(((ni) as i64)).max == 0i64) || (nodes.get(((child_4) as i64)).maxL == 0i64)) {
                unsafe { (*nodes.ptr(((ni) as i64))).maxL = 0i64; }
            } else {
                if (nodes.get(((ni) as i64)).max == infinite) {
                    unsafe { (*nodes.ptr(((ni) as i64))).maxL = lenInf; }
                } else {
                    unsafe { (*nodes.ptr(((ni) as i64))).maxL = satMul(nodes.get(((ni) as i64)).max, nodes.get(((child_4) as i64)).maxL); }
                }
            }
        }
    }
