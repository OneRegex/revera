...
This module is a cleanroom Go implementation of [`docs/POSIX-1-2024-ERE-SPECIFICATION.md`](../docs/POSIX-1-2024-ERE-SPECIFICATION.md).
...
- `revera` is the engine and its regcomp-style API.
- `revera/locale` supplies `LC_CTYPE` and the ERE-relevant `LC_COLLATE` operations from embedded CLDR 48.2 data.
...
- `gen/genlocale` rebuilds `locale/data.bin` from the C data file `src/rv_locale_data.inc` at the repository root.
...
- `Compile` flags are `ICase`, `Newline`, `NoSub` and `Minimal`.
  `REG_EXTENDED` is implicit, because the module implements only EREs.
...
- An error carries the POSIX code, such as `BadPat`, `EBrack` or `ECollate`, and the byte offset in the pattern.
...
- A second differential compares whole-match selection with the `regexp.CompilePOSIX` of Go, on long subjects, inside the common subset.
...
- The locale blob is generated from the C tables at the repository root.
  A lookup dump over 19 locale selections came out bit-identical to the C implementation.
