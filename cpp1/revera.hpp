...
//
// This header is the public surface of the revera engine. The
// engine itself, in engine.hpp, is generated from a Vego program
// and speaks in arenas, raw views and numeric flags. Nothing here
// needs those, and this header does not include them; a caller who
// wants the execution flags of regexec() includes engine.hpp and
// works in namespace revera::engine.
//
...
// defaults to POSIX.
//
// Compilation and search failures throw revera::Error. A subject
// that simply does not match is not a failure: the search returns
// an empty optional.
...
// One match and the spans of its capturing groups. Element 0 is the
// whole match. A group that took no part in the match reads as
// std::nullopt.
...
//
// A Locale is immutable and cheap to copy. The default is POSIX.
class Locale {
  public:
    Locale();

    // Resolves a CLDR locale name against the embedded data, for
    // example Locale::open("cs"). An empty collation type takes the
    // standard collation of the locale. The result is empty when
    // the name or the collation type is unknown.
...
struct Options {
    ...
    // Regex::matches still works; every other search throws with
    // Failure::NoCaptures.
...
//
// Searching is const and keeps no state between calls, so one Regex
...
class Regex {
  public:
    ...
    // Returns subject with at most limit matches replaced. The rest
    // of the subject stays as it is.
