...
One POSIX regex engine in Go, Rust, Zig, C, C++ and TypeScript, cross-checked for one behavior and backed by machine-checked Lean semantics and proofs.
...
A compiler exports it as Vego IR, and printers turn that IR into the Rust, Zig, C++, C and TypeScript engines.
...
| TypeScript | npm package `@oneregex/revera`               | `ts/`         |
...
The generated engine files of the other languages keep their names everywhere: `engine.rs`, `engine.zig`, `engine.ts`, `engine.hpp` with `engine.cpp`, and `engine.h` with `engine.c`.
...
- [`rust/`](rust/), [`zig/`](zig/), [`ts/`](ts/) and [`native/`](native/) are the engine printed into each target language, with a hand-written public API in the shape that language expects.
  `native/` is one CMake package with the C and C++ libraries.
  `ts/` runs under Node with no build step, and maps the 64-bit integers of the engine onto `number` and `bigint` so that nothing is approximated.
...
(cd ts && npm install && npm run check && npm test)
(cd lean && lake build)
...
Set `GENERATION_TARGETS=rust`, `zig`, `ts`, `cpp`, `c`, or a comma-separated selection to limit the target sources.
