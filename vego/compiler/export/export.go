// Package export checks that a package conforms to the Vego subset.
// It then translates the package into the JSON representation that SPECIFICATION.md defines.
//
// Files whose names end in _host.go or _test.go are host files, and the exporter skips them.
// Every construct outside the subset is reported with a file and a line, and no JSON comes out then.
package export
...
func (c *checker) checkPackageName(name *ast.Ident) {
	if compiler.ReservedPackageName(name.Name) {
...
// Package checks the Vego package in dir.
// It returns the JSON form of the package, or the list of subset violations.
func Package(dir string) ([]byte, []string, error) {
