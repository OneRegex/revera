// Package zig prints the Vego JSON form as one Zig source file.
...
// Emit prints the program as one Zig source file.
...
func (g *gen) zero(t *compiler.Type) string {
	...
	case compiler.KBool:
		return "false"
	case compiler.KStr, compiler.KSlice, compiler.KStruct:
		return ".{}"
	case compiler.KArray:
...
func (g *gen) pinOpAssign(s *compiler.Stmt) bool {
	return (s.Op == "/=" || s.Op == "%=") && compiler.Impure(s.Lhs[0])
}

func (g *gen) opAssignPlace(s *compiler.Stmt, lhs string) string {
	val := g.expr(s.Value)
	switch s.Op {
	case "+=", "-=", "*=":
