// Package c prints the Vego JSON form as a C11 header and source pair.
...
type Options struct {
	// HeaderName is the file name the source includes; the default is engine.h.
	HeaderName string
	// Prefix is the global identifier prefix; the default is the Vego package name.
	Prefix string
}
...
type gen struct {
	...
	sliceOrder []string
	sliceTypes map[string]*compiler.Type
	arrayOrder []string
	arrayTypes map[string]*compiler.Type
}
...
func (g *gen) mangle(t *compiler.Type) string {
	switch t.K {
	case compiler.KBool:
		return "bool"
	case compiler.KU8:
...
func (g *gen) sliceName(t *compiler.Type) string {
	return g.global(g.mangle(t))
}

func (g *gen) arrName(t *compiler.Type) string {
	return g.global(g.mangle(t))
}
...
func (g *gen) decl(t *compiler.Type, name string) string {
	s := g.typ(t)
...
func (g *gen) collect() {
	g.sliceTypes = map[string]*compiler.Type{}
	g.arrayTypes = map[string]*compiler.Type{}
	var addType func(t *compiler.Type)
	addType = func(t *compiler.Type) {
		if t == nil {
			return
		}
		switch t.K {
		...
		case compiler.KTuple:
			addType(t.Tup[0])
			addType(t.Tup[1])
		}
	}
	fe := func(e *compiler.Expr) {
		addType(e.Typ)
		addType(e.TypeRef)
	}
	fs := func(s *compiler.Stmt) {
		addType(s.TypeRef)
	...
	for _, f := range g.p.Funcs {
		...
		compiler.WalkBody(f.Body, fe, fs)
	}
}
...
func (g *gen) declPtr(t *compiler.Type, name string) string {
	s := g.typ(t)
	if strings.HasSuffix(s, "*") {
		return s + "*" + name
	}
...
func (g *gen) emitTypeDefs(h *strings.Builder) {
	...
	var visitStruct func(s *compiler.StructDecl)
	visitArray := func(t *compiler.Type) {}
	...
	visitStruct = func(s *compiler.StructDecl) {
		key := "struct " + s.Name
		switch state[key] {
		case 2:
			return
...
func (g *gen) declArr(elem *compiler.Type, name string, n int64) string {
...
func (g *gen) eqCall(t *compiler.Type, a, b string) string {
	switch t.K {
	case compiler.KStr:
...
func (g *gen) emitEqHelpers(h *strings.Builder) {
	...
	var decls []eqDecl
	for _, s := range g.p.Types {
		t := &compiler.Type{K: compiler.KStruct, Name: s.Name}
		if g.comparable(t) {
...
func (g *gen) emitArrayHelpers(h *strings.Builder) {
	for _, key := range g.arrayOrder {
		t := g.arrayTypes[key]
		st := &compiler.Type{K: compiler.KSlice, Elem: t.Elem}
...
func (g *gen) signature(f *compiler.FuncDecl) string {
	...
	case 2:
		ret = g.global(compiler.TupName([2]*compiler.Type{f.Results[0], f.Results[1]}))
	}
	sep := " "
...
func (g *gen) hoist(t *compiler.Type, value string) string {
	tmp := g.newTmp()
...
func (g *gen) stmt(s *compiler.Stmt, depth int) {
	...
	case "return":
		...
		default:
			...
			g.flushPre()
			ret := g.global(compiler.TupName([2]*compiler.Type{g.fn.Results[0], g.fn.Results[1]}))
			g.indent(depth)
...
func (g *gen) tupleOf(e *compiler.Expr) string {
	if e.Typ == nil || e.Typ.K != compiler.KTuple {
		fatal("expression is not a two-result call")
	}
	return g.global(compiler.TupName(e.Typ.Tup))
}

// pinPlace evaluates an assignment target once, into a pointer temporary, and returns the dereference.
func (g *gen) pinPlace(l *compiler.Expr) string {
	lstr := g.expr(l)
	tmp := g.newTmp()
...
func (g *gen) emitAssign2(s *compiler.Stmt, depth int) {
	impure := false
	for _, l := range s.Lhs {
		if !(l.K == "ident" && l.Name == "_") && compiler.Impure(l) {
			impure = true
		}
	}
...
func (g *gen) shiftLeft(t *compiler.Type, x, y string) string {
	var u string
	switch t.Width() {
	case 64:
		u = "uint64_t"
...
func (g *gen) sliceExpr(e *compiler.Expr) string {
	hoisting := compiler.Impure(e.X) || compiler.Impure(e.Lo) || compiler.Impure(e.Hi)
	arrayBase := ""
	x := ""
	...
	render := func(b *compiler.Expr) string {
		s := g.expr(b)
		if hoisting {
			s = g.hoist(b.Typ, s)
		}
...
func (g *gen) renderArgs(args []*compiler.Expr) []string {
...
func (g *gen) binary(e *compiler.Expr) string {
	...
	if (compiler.Impure(e.X) && !constish(e.Y)) || (compiler.Impure(e.Y) && !constish(e.X)) {
		...
		clone.X = &compiler.Expr{K: "ident", Name: hx, Typ: e.X.Typ}
		clone.Y = &compiler.Expr{K: "ident", Name: hy, Typ: e.Y.Typ}
...
func (g *gen) logical(e *compiler.Expr) string {
	x := g.expr(e.X)
	cp := g.save()
	y := g.expr(e.Y)
	if len(g.pre) == cp.pre {
...
func (g *gen) renderInits(vals []*compiler.Expr) []string {
	impure, others := 0, 0
	for _, v := range vals {
		if constish(v) {
			continue
		}
		if compiler.Impure(v) {
			impure++
		} else {
			others++
		}
...
func (g *gen) staticInit(e *compiler.Expr) string {
	switch e.K {
	case "int", "char":
		return g.intLiteral(e)
	case "bool":
