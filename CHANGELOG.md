...
The engine release number is shared by the Go, Rust, Zig, C and C++ implementations.
...
- The Revera engine, a clean-room POSIX.1-2024 ERE implementation written in Vego and printed into Go, Rust, Zig, C and C++.
...
- The Go modules `github.com/oneregex/revera/go` and `github.com/oneregex/revera/vego`, the `revera` crate, the `revera` Zig package and the `revera` CMake package with `Revera::C` and `Revera::CXX`.
