...
//!
//! This module is the public surface of the revera engine. The
//! engine itself, in engine.zig, is generated from a Vego program
//! and speaks in arenas, raw views and numeric flags. Nothing here
//! needs those, but engine.zig stays reachable for a caller who
//! wants the execution flags of regexec().
//!
...
//! collating elements and equivalence classes from a Locale, which
//! defaults to POSIX.
...
/// Every way compilation or a search can fail. The names follow the
/// <regex.h> error constants.
pub const Error = error{
    ...
    InvalidInterval,
    /// A range like [z-a] runs backwards, or its endpoint is not a
    /// single character.
    InvalidRange,
    ...
    /// The expression was compiled with no_captures, and the call
    /// needs match offsets.
    NoCaptures,
...
/// One matched span of a subject. It borrows the subject, so it
/// stays valid only as long as the subject does.
...
/// match reads as null. The group list is owned, so every Captures
/// value needs deinit.
pub const Captures = struct {
    ...
    /// Returns group i, or null when it took no part in the match
    /// or does not exist.
...
pub const Locale = struct {
    ...
    /// example Locale.open(gpa, "cs", ""). An empty collation type
    /// takes the standard collation of the locale. The result is
    /// null when the name or the collation type is unknown.
    ///
    /// The allocator backs the lookup only. The locale it returns
    /// reads the embedded data and owns nothing.
    ...
    /// Returns every locale name the embedded data carries. The
    /// names point into that data; the caller frees only the outer
    /// slice.
...
pub const Options = struct {
    ...
    /// Gives ^ and $ their line meaning and stops dot and negated
    /// brackets on a newline, like REG_NEWLINE.
    newline_sensitive: bool = false,
    /// Compiles for a yes-or-no answer only, like REG_NOSUB.
    /// isMatch still works; every other search reports
    /// error.NoCaptures.
    ...
    /// The locale the bracket expressions read. Null means POSIX.
    locale: ?Locale = null,
    /// Receives the byte offset in the pattern where compilation
    /// stopped. Some failures have no position, and those leave
    /// the slot as the caller set it.
...
///
/// Searching takes a const pointer and keeps no state between
/// calls. One Regex therefore serves any number of threads. The
/// allocator it was compiled with must be thread safe, because
/// every search takes its scratch memory from that allocator.
pub const Regex = struct {
    /// The arena owns the compiled program. Compile writes it once
    /// and every search only reads it. It also carries the
    /// allocator it was built from, which every search borrows.
    arena: std.heap.ArenaAllocator,
    inner: engine.Regexp,
    groups: usize,

    /// Compiles a pattern. The Regex owns memory until deinit.
    ...
    // backing is the allocator compile was given. Every search
    // borrows it for scratch memory.
    ...
    /// Returns the leftmost-longest match, or null when there is
    /// none.
    ...
    /// when there is none. The result owns memory until deinit.
    pub fn captures(
        self: *const Regex,
        subject: []const u8,
    ...
    /// Returns an iterator over the non-overlapping matches, left
    /// to right. The Regex and the subject must both outlive the
    /// iterator.
    pub fn matches(self: *const Regex, subject: []const u8) Error!MatchIterator {
        return .{ .scan = try Scan.init(self, subject) };
    }

    /// Returns an iterator over the non-overlapping matches with
    /// their groups. Every value it yields owns memory until
    /// deinit. The Regex and the subject must both outlive the
    /// iterator.
    ...
    /// like the sed command s///g. The caller frees the result.
    ///
    /// In replacement, & stands for the whole match and \1 through
    /// \9 for one group. A backslash escapes the next character, so
    /// \& and \\ are literal.
    pub fn replaceAll(
        self: *const Regex,
        subject: []const u8,
    ...
    /// max_input bytes. An application compares the figures against
    /// its budget and refuses the expression before it ever runs.
...
// iterators wrap it. One step fills a slice of the caller's arena,
// which lets the caller free the arena as soon as it has copied
// the offsets out.
const Scan = struct {
    re: *const Regex,
    subject: []const u8,
    // The engine takes a mutable pointer to the compiled program,
    // so a scan walks a copy of the header. One copy serves every
    // step of the scan.
    walk: engine.Regexp,
...
///
/// Every figure saturates at 1 << 62, which marks a bound too large
/// to be useful.
