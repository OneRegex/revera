// Probe runner for the Zig instantiation.
...
pub fn main(init: std.process.Init) !void {
    ...
    try w.print("bytes = {d} {d}\n", .{ try p.BytesProbe(mem, vg.lit("hello")), try p.BytesProbe(mem, .{}) });
    var c1: p.Counter = .{};
    try w.print("range = {d}\n", .{try p.RangeProbe(mem, &c1)});
    try w.print("rangeval = {d}\n", .{p.RangeValProbe(try vg.sliceOf(mem, i32, &[_]i32{ 3, 5, 7 }))});
    ...
    var c2: p.Counter = .{};
    try w.print("orderargs = {d}\n", .{try p.OrderArgs(mem, &c2)});
    var c3: p.Counter = .{};
    try w.print("orderbinary = {d}\n", .{try p.OrderBinary(mem, &c3)});
    var c4: p.Counter = .{};
    try w.print("orderindex = {d}\n", .{try p.OrderIndex(mem, &c4)});
    try w.print("spare = {d}\n", .{try p.SpareProbe(mem)});
    try w.print("nil = {d}\n", .{try p.NilProbe(mem)});
